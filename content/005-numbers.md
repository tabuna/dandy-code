...
В этом примере магическое число `1` используется для определения статуса активности.
Но если я из мира Linux, то могу посчитать, `1`  так же как коды выхода программы, где 1 означает ошибку.


Однако, такой код может быть непонятным для других разработчиков, и в долгосрочной перспективе это усложняет поддержку и изменение кода.

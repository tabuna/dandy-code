...
Такой код корректен: метод выполнится только один раз. 
Однако такое использование оператора `??=`(null coalescing assignment) может быть неочевиден для неподготовленного читателя.
Он означает: "если переменная `$moon` ещё не определена (или равна `null`), присвой ей значение". 
Это удобно, но внутри цикла может выглядеть как «магия».
Чтобы избежать скрытой логики, лучше сделать намерение явным:
